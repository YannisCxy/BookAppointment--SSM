                        ...
                        <td>
                            <c:if test="${orders.status eq '1'}">
                                <button type="button" class="btn btn-sm">
                                    <a href="${pageContext.request.contextPath}/cancelAppoint.do?username=${username}&status=${orders.status}&id=${orders.id}">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <nav aria-label="Page navigation">
                <ul class="pagination">
                    <li>
                        <a href="${pageContext.request.contextPath}/queryByStatus.do?username=${username}&status=${nowstatus}&pn=1">首页</a>
                    </li>
                    <li>
                        <c:if test="${pageInfo.hasPreviousPage}">
                            <a href="${pageContext.request.contextPath}/queryByStatus.do?username=${username}&status=${nowstatus}&pn=${pageInfo.pageNum-1}"
                               aria-label="Previous">
                    ...
                    <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                        <c:if test="${page_num==pageInfo.pageNum}">
                            <li class="active"><a
                                    href="${pageContext.request.contextPath}/queryByStatus.do?username=${username}&status=${nowstatus}&pn=${page_num}">${page_num}</a>
                            </li>
                        </c:if>
                        <c:if test="${page_num!=pageInfo.pageNum}">
                            <li>
                                <a href="${pageContext.request.contextPath}/queryByStatus.do?username=${username}&status=${nowstatus}&pn=${page_num}">${page_num}</a>
                            </li>
                        </c:if>
                    </c:forEach>
                    ...
                    <li>
                        <a href="${pageContext.request.contextPath}/queryByStatus.do?username=${username}&status=${nowstatus}&pn=${pageInfo.pages}">末页</a>
                    </li>
                </ul>
            </nav>
        </div>
